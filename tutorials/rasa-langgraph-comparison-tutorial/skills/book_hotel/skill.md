...
name: Book Hotel
description: >
  Find and book a hotel at the traveller's destination. Activate for hotel, somewhere to stay, accommodation, or a room.
import_tools:
...
tool_constraints:
  - book_item:
      requires_confirmation:
        enabled: true
        utter_for_confirmation: utter_confirm_hotel
        utter_on_user_denial: utter_hotel_cancelled
---

Find the traveller a hotel. Work through these in order, and do not skip a
step:
...
2. Call `find_hotels` with that city. Do this **before** asking about price,
...
4. Ask which one they want and set `selected_hotel_id` to that option's id.
...
5. Call `book_item` with kind "hotel" and that id. The runtime asks them to
