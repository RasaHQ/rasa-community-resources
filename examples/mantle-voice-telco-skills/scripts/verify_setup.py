#!/usr/bin/env python3
"""Pre-flight diagnostics for the Telano voice telecom care agent.
...
def check_agent_structure(report: Report) -> None:
    ...
    if len(seeds) >= 3:
        ...
        report.error(
            ...
            "Restore data/source/{customers,billing,routers}.json",
        )
...
def check_demo_data(report: Report) -> None:
    ...
    try:
        ...
        bills = db.run_query(
            "SELECT date FROM billing WHERE customer_id = ?",
            (customer_id,),
            one_record=False,
        )
        routers = db.run_query(
            "SELECT device_id FROM routers WHERE customer_id = ?",
            (customer_id,),
            one_record=False,
        )
        ok(
            f"{DEMO_USERNAME} ({DEMO_CUSTOMER_ID}): "
            f"{len(bills or [])} bills, {len(routers or [])} routers"
        )
        info("Run 'make show-demo-data' for the numbers to use during the demo")
    except Exception as exc:
        report.error(f"Demo telco DB failed to seed: {exc}", "make reset-db")
...
def main() -> None:
    print(f"\n{BOLD}{BLUE}{'=' * 62}{RESET}")
    print(f"{BOLD}{BLUE}  Telano — Pre-flight diagnostics{RESET}")
    ...
    if report.errors == 0 and report.warnings == 0:
        ...
        print()
        print(f"  {MAGENTA}Talk to Telano:{RESET}")
        ...
        print(f"    {GREEN}make reset-db{RESET}         reseed the demo telco DB")
