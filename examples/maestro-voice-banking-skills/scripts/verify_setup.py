#!/usr/bin/env python3
"""Pre-flight diagnostics for the Rasano voice banking agent.
...
_TOOL_SIG = re.compile(
    r"@tool\([^\n]*\)\s*\n\s*async\s+def\s+(\w+)\s*\((.*?)\)\s*->",
    re.DOTALL,
)


def _tool_signatures(path: Path) -> list[tuple[str, list[str]]]:
    """Return (tool_name, llm_facing_args) for every @tool in *path*.

    The injected ``context`` parameter is excluded — it never appears in the
    LLM tool schema.
    """
    signatures: list[tuple[str, list[str]]] = []
    for name, raw_params in _TOOL_SIG.findall(path.read_text()):
        args = []
        for chunk in raw_params.split(","):
            param = chunk.strip().split(":")[0].split("=")[0].strip()
            if param and param != "context":
                args.append(param)
...
def _check_tools(report: Report) -> None:
    shared_paths = sorted((PROJECT_ROOT / "tools").glob("*.py"))
    local_paths = sorted(PROJECT_ROOT.glob("skills/*/tools.py"))

    if not shared_paths and not local_paths:
        report.error("No tools found under tools/*.py or skills/*/tools.py")
        return

    shared: list[tuple[str, list[str]]] = []
    for path in shared_paths:
        shared.extend(_tool_signatures(path))
    local: list[tuple[str, list[str]]] = []
    for path in local_paths:
        local.extend(_tool_signatures(path))

    if shared:
        ok(f"{len(shared)} shared tools  {DIM}(tools/: {', '.join(n for n, _ in shared)}){RESET}")
    else:
        report.warning("No shared tools under tools/*.py")

    if local:
        by_skill = ", ".join(sorted({p.parent.name for p in local_paths}))
        ok(f"{len(local)} skill-local tools  {DIM}(auto-discovered in {by_skill}){RESET}")
    else:
        info("No skill-local tools yet (skills/<name>/tools.py)")

    # Regression guard: the invalid `@tool.` reference token must never return to
    # skill prose — Maestro names tools in plain prose (only @skill./@block. use @).
    offenders = [
        p.parent.name
        for p in sorted(PROJECT_ROOT.glob("skills/*/skill.md"))
        if "@tool." in p.read_text()
    ]
    if offenders:
        report.error(
            f"Invalid '@tool.' reference in skill prose: {', '.join(offenders)}",
            "Name tools in plain prose (e.g. 'Call list_accounts'); there is no @tool. token",
        )
    else:
        ok("No '@tool.' reference tokens in skill prose")

    # Zero-arg tools are valid schemas, but the LLM should not be relied on to call
    # them — load fixed demo data deterministically at session start (execute_tool).
    zero_arg = [n for n, args in shared + local if not args]
    if zero_arg:
        info(
            f"Zero-argument tools: {', '.join(zero_arg)}  "
            f"{DIM}(prefer execute_tool at session start over LLM calls){RESET}"
        )
...
def check_agent_structure(report: Report) -> None:
    ...
    if (PROJECT_ROOT / "lib" / "database.py").is_file():
        ok("lib/database.py  (demo bank helpers)")
    else:
    ...
    if len(seeds) >= 8:
        ...
        report.error(
            f"Expected 8 seed files in data/source/, found {len(seeds)}",
...
def check_demo_data(report: Report) -> None:
    section("Demo bank data")
    try:
        from lib.database import DEMO_USERNAME, Database, get_user_id
    except ImportError as exc:
        report.error(f"Cannot import the demo bank: {exc}", "make install")
        return
    ...
    try:
        ...
        cards = db.run_query(
            "SELECT number FROM cards WHERE user_id = ?", (user_id,), one_record=False
        )
        payees = db.run_query(
            "SELECT name FROM payees WHERE user_id = ?", (user_id,), one_record=False
        )
        ok(
            f"{DEMO_USERNAME}: {len(accounts or [])} accounts, "
            f"{len(cards or [])} cards, {len(payees or [])} payees"
        )
        info("Run 'make show-demo-data' for the numbers to use during the demo")
    except Exception as exc:
        report.error(f"Demo bank failed to seed: {exc}", "make reset-db")
...
def main() -> None:
    print(f"\n{BOLD}{BLUE}{'=' * 62}{RESET}")
    print(f"{BOLD}{BLUE}  🏦  Rasano — Pre-flight diagnostics{RESET}")
    ...
    if report.errors == 0 and report.warnings == 0:
        ...
        print()
        print(f"  {MAGENTA}Talk to Rasano:{RESET}")
        ...
        print(f"    {GREEN}make reset-db{RESET}         reseed the demo bank")
