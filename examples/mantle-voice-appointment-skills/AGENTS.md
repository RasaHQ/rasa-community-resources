...
**Deepgram** ASR + TTS. It targets `rasa-pro==3.20.0.dev20260916190743` on `gpt-5.2`.
...
- `endpoints.yml` — platform services read by `rasa train` / `rasa inspect`
  (contextual response rephraser, model groups). Mantle still uses this file;
...
- `@tool.<name>` is **not** a Mantle construct. Call tools in plain prose
