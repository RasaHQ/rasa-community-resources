...
SECRETS = [
    ...
    ("SPEECHMATICS_API_KEY", "Speech-to-text (custom engine)",
     "https://portal.speechmatics.com/", False),
...
REQUIRED_FILES = [
    ...
    "lib/bank.py", "tools/profile.py", "engines/speechmatics.py",
]
...
def main() -> int:
    ...
    print(f"\n{DIM}Vela (Speechmatics) — pre-flight{RESET}\n")
    ...
    try:
        ...
        if "speechmatics" not in str(asr).lower():
            print(f"  {YELLOW}note{RESET}  this resource is about Speechmatics ASR; asr is {asr!r}")
    except ImportError:
